...
struct SessionState(Mutex<UserSession>);
...
#[tauri::command]
fn verify_pin(pin: String, pin_hash: String) -> Result<bool, String> {
...
#[tauri::command]
fn get_session(state: tauri::State<'_, SessionState>) -> UserSession {
    state.0.lock().unwrap().clone()
}

#[tauri::command]
fn logout(state: tauri::State<'_, SessionState>) {
    let mut session = state.0.lock().unwrap();
...
pub fn run() {
    ...
    tauri::Builder::default()
        .manage(SessionState(Mutex::new(UserSession::default())))
        .plugin(
        ...
        .invoke_handler(tauri::generate_handler![
            hash_pin, verify_pin, login, get_session, logout
        ])
